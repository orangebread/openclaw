...
- Sparkle signing key path set in the environment as <code>SPARKLE_PRIVATE<wbr>\_KEY<wbr>\_FILE</code> (path to your Sparkle ed25519 signing key; public key baked into Info.plist). If it is missing, check `~/.profile`.
- Notary credentials (keychain profile or API key) for `xcrun notarytool` if you want Gatekeeper-safe DMG/zip distribution.
  - We use a Keychain profile named `openclaw-notary`, created from App Store Connect API key env vars in your shell profile (for example <code>APP_STORE_CONNECT_API<wbr>\_KEY<wbr>\_P8</code>, <code>APP_STORE_CONNECT_KEY<wbr>\_ID</code>, <code>APP_STORE_CONNECT_ISSUER<wbr>\_ID</code>). Example:
    - `xcrun notarytool store-credentials "openclaw-notary" --key /tmp/openclaw-notary.p8 --key-id "<key-id>" --issuer "<issuer-id>"`
...
```bash
# Ensure SPARKLE_PRIVATE_KEY_FILE is set, then run:
scripts/make_appcast.sh dist/OpenClaw-2026.2.14.zip https://raw.githubusercontent.com/openclaw/openclaw/main/appcast.xml
```
