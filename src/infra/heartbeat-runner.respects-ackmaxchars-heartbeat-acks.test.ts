...
describe("resolveHeartbeatIntervalMs", () => {
  ...
  async function withTempTelegramHeartbeatSandbox<T>(
    ...
    const telegramTokenKey = ["TELEGRAM", "BOT", "TOKEN"].join("_");
    const prevTelegramToken = process.env[telegramTokenKey];
    process.env[telegramTokenKey] = "";
    try {
      return await fn({ tmpDir, storePath, replySpy });
    } finally {
      replySpy.mockRestore();
      if (prevTelegramToken === undefined) {
        delete process.env[telegramTokenKey];
      } else {
        process.env[telegramTokenKey] = prevTelegramToken;
      }
