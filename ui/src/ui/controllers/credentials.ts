...
async function buildDisconnectWarning(state: CredentialsState, profileId: string): Promise<string> {
  if (!state.client || !state.connected) return "";
  try {
    const snapshot = (await state.client.request("config.get", {})) as ConfigSnapshot | undefined;
    if (!snapshot?.valid || !isPlainRecord(snapshot.config)) return "";
    const cfg = snapshot.config as Record<string, unknown>;

    const impacts: string[] = [];

    const auth = cfg.auth;
    if (isPlainRecord(auth) && isPlainRecord(auth.profiles) && profileId in auth.profiles) {
      impacts.push(`Referenced by config auth.profiles["${profileId}"].`);
    }
    ...
    if (lockedTextAgents.length) {
      impacts.push(`Locked as authProfileId for agent(s): ${lockedTextAgents.join(", ")}.`);
    }
    if (lockedImageAgents.length) {
      impacts.push(`Locked as imageAuthProfileId for agent(s): ${lockedImageAgents.join(", ")}.`);
    }

    if (!impacts.length) return "";
    return `\n\nWarning: This profile is referenced in config.\n${impacts.map((i) => `- ${i}`).join("\n")}\n\nDisconnecting may cause failures until you update defaults or agent locks.`;
  } catch {
    return "";
  }
}
...
function resetAuthFlowAnswerForStep(step: AuthFlowStep | null): unknown {
  ...
  if (step.type === "confirm") return typeof step.initialValue === "boolean" ? step.initialValue : false;
...
async function refreshWizardOwnership(state: CredentialsState): Promise<WizardCurrentResult | null> {
...
async function refreshAuthFlowOwnership(state: CredentialsState): Promise<AuthFlowCurrentResult | null> {
  if (!state.client || !state.connected) return null;
  const res = (await state.client.request("auth.flow.current", {})) as AuthFlowCurrentResult | undefined;
  if (!res?.running) {
...
async function fetchWizardStep(state: CredentialsState, sessionId: string): Promise<WizardStep | null> {
  const res = (await state.client?.request("wizard.next", { sessionId })) as WizardNextResult | undefined;
  if (!res) return null;
...
async function fetchAuthFlowStep(state: CredentialsState, sessionId: string): Promise<AuthFlowStep | null> {
  const res = (await state.client?.request("auth.flow.next", { sessionId })) as AuthFlowNextResult | undefined;
  if (!res) return null;
  if (res.done || res.status && res.status !== "running") {
    clearAuthFlowState(state);
    ...
    try {
      while (true) {
        ...
        try {
          ...
          if (state.credentialsWizardRunning && state.credentialsWizardOwned && state.credentialsWizardSessionId) {
            if (!state.credentialsWizardStep) {
              await fetchWizardStep(state, state.credentialsWizardSessionId);
            }
          }
          if (state.credentialsAuthFlowRunning && state.credentialsAuthFlowOwned && state.credentialsAuthFlowSessionId) {
...
async function applyAuthFlowConfigPatch(state: CredentialsState, patch: unknown) {
  ...
  try {
    ...
    const shouldRetry = msg.includes("config changed since last load") || msg.includes("config base hash required");
    if (!shouldRetry) {
...
export async function deleteCredentialsProfile(state: CredentialsState, profileId: string) {
  if (!state.client || !state.connected) return;
  if (state.credentialsSaving) return;
  ...
  const warning = await buildDisconnectWarning(state, trimmed);
  const countLine =
    provider && providerCount && providerCount > 1
      ? `\n\nThis will remove 1 of your ${providerCount} ${provider} credential profiles.`
      : "";
  const confirmed = window.confirm(
    `Disconnect credential "${trimmed}"? This deletes the stored credential material.${countLine}${warning}`,
  );
  if (!confirmed) return;
  state.credentialsSaving = true;
  state.credentialsError = null;
  try {
    ...
    try {
      ...
      const shouldRetry =
        message.includes("auth base hash required") || message.includes("auth store changed since last load");
...
export async function advanceCredentialsAuthFlow(state: CredentialsState) {
  ...
  const shouldClearSensitive =
    submittedStep.type === "text" && submittedStep.sensitive && typeof state.credentialsAuthFlowAnswer === "string";
  try {
    const hadProviderProfilesBefore = state.credentialsAuthFlowHadProviderProfilesBefore;
    const sessionId =
...
export async function advanceCredentialsWizard(state: CredentialsState) {
  ...
  const shouldClearSensitive =
    submittedStep.type === "text" && submittedStep.sensitive && typeof state.credentialsWizardAnswer === "string";
  try {
    const sessionId =
